...
class StockBot:
    def __init__(self, triggers_file=None):
        if triggers_file is None:
            triggers_file = os.path.join(os.path.dirname(__file__), 'stockday.txt')
        self.trigger_phrases = self.read_triggers_from_file(triggers_file)
    ...
    def read_triggers_from_file(self, filename):
    ...
    def get_stock_price(self, symbol):
        try:
            ...
            previous_close = int(history['Close'].iloc[-2])
            current_price = int(stock.history(period='1d')['Close'].iloc[-1])
            return current_price, previous_close
        except Exception as e:
            print(f"Error fetching stock data: {e}")
            return None, None

    def get_top_performing_companies(self, num_companies=10):
        performance_data = {}
        for ticker, company_name in self.company_ticker_mapping.items():
            ticker_with_extension = f"{ticker}.NS"
            current_price, previous_close = self.get_stock_price(ticker_with_extension)
            if current_price is not None and previous_close is not None:
                price_difference = current_price - previous_close
                performance_data[company_name] = price_difference
        
        sorted_performance = sorted(performance_data.items(), key=lambda x: x[1], reverse=True)
        
        top_companies = []
        for i, (company_name, price_difference) in enumerate(sorted_performance[:num_companies], 1):
            top_companies.append(f"{i}. {company_name}: Growth of: {price_difference} ₹")
    ...
    def bot_response(self, user_input):
        for trigger in self.trigger_phrases:
            if trigger in user_input.lower():
                ...
                top_companies = self.get_top_performing_companies()
                response = "Here are the top performing companies for the day:<br>"
                response += "<br>".join(top_companies)
                return response
...
if __name__ == "__main__":
    bot = StockBot()
    print("You can start chatting with the StockBot. Type 'bye' to exit.")
    while True:
        ...
        if user_input.lower() == 'bye':
            ...
            print("StockBot:", bot.bot_response(user_input))
