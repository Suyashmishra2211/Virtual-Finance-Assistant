...
class stockbot:
    def __init__(self):
        self.triggers_file = os.path.join(os.path.dirname(__file__), 'stockweek.txt')
        self.triggers = self.load_triggers(self.triggers_file)
        if not self.triggers:
            print("Chatterbot: No triggers loaded. Please check your triggers file.")
    ...
    def load_triggers(file_path):
        try:
            ...
            print(f"Error: Triggers file '{file_path}' not found.")
            return []
    
    @staticmethod
    def get_stock_prices(symbols):
        try:
            stock_data = yf.download(symbols, start=pd.Timestamp.now() - pd.Timedelta(days=7), end=pd.Timestamp.now())
    ...
    def get_top_stocks(self):
        ...
        symbols = [f"{ticker}.NS" for ticker in company_ticker_mapping.keys()]
        stock_prices = self.get_stock_prices(symbols)
        if stock_prices is not None:
            weekly_price_change = stock_prices.iloc[-1] - stock_prices.iloc[0]
            sorted_changes = weekly_price_change.sort_values(ascending=False)
            top_n_changes = sorted_changes.head(10)

            top_stocks = []
            for ticker, price_change in top_n_changes.items():
                company_name = company_ticker_mapping[ticker[:-3]]
                top_stocks.append((company_name, price_change))
    ...
    def respond(self, user_input):
        if any(trigger in user_input for trigger in self.triggers):
            top_stocks = self.get_top_stocks()
            if top_stocks is not None:
                response = "Here are the top 10 performing stocks of the week:<br>"
                for rank, (company_name, price_change) in enumerate(top_stocks, start=1):
                    response += f"{rank}. {company_name}: {price_change:.2f} INR<br>"
            else:
                response = "Sorry, I couldn't fetch the stock data. Please try again later."
        else:
            response = "I'm sorry, I didn't understand that. Please ask me for the top performing stocks of the week."
        return response


if __name__ == "__main__":
    chatterbot_instance = stockbot()
    print("Chatterbot: Hello! How can I assist you?")
    while True:
        user_input = input("You: ").lower()
        if user_input == 'exit':
            print("Chatterbot: Exiting the program...")
            break
        else:
            print("Chatterbot:", chatterbot_instance.respond(user_input))
