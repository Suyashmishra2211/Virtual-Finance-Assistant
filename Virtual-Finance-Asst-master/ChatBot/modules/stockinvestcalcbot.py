...
class StockInvestmentChatterbot:
    def __init__(self):
        pass
    
    def respond(self, input_text):
        initial_investment, duration_days = self.extract_investment_details(input_text)
        
        if initial_investment is not None and duration_days is not None:
            best_company, max_return = self.find_best_company(initial_investment, duration_days)
            if best_company is not None:
                return f"The best company to invest at the moment for a duration of {duration_days} days with an initial investment of ₹{initial_investment} is {best_company}.With an expected return of: ₹{max_return:.2f}"
            else:
                return "Sorry, unable to find the best company for your investment."
        elif "exit" in input_text.lower():
            return "Goodbye!"
        else:
            return "I'm sorry, I don't understand that."

    def convert_to_days(self, duration, unit):
    ...
    def calculate_expected_return(self, data, current_price, initial_investment, duration_days):
        try:
    ...
    def extract_investment_details(self, input_text):
        ...
        investment_match_1 = re.search(r"(?:₹\s*)(\d+(?:,\d{3})*(?:\.\d+)?)\b", input_text, re.IGNORECASE)
        if investment_match_1:
            initial_investment = float(investment_match_1.group(1).replace(",", ""))
        else:
            # Second regex to check for terms like "rs", "rupee", "rupees", "INR" after any value
            investment_match_2 = re.search(r"(\d+(?:,\d{3})*(?:\.\d+)?)\s*(?:rs|rupee|rupees|INR)\b", input_text, re.IGNORECASE)
            if investment_match_2:
                initial_investment = float(investment_match_2.group(1).replace(",", ""))
        
        # Regex to match duration in numeric form
        duration_match = re.search(r"(\d+(?:\.\d+)?)\s*(year[s]?|month[s]?|day[s]?|yr[s]?|mon|da)", input_text, re.IGNORECASE)
        if duration_match:
            duration = float(duration_match.group(1))
            unit = duration_match.group(2).lower()
            duration_days = self.convert_to_days(duration, unit)
        
        if not duration_days:
            # Regex to match duration in alphabetic form
            ...
            if term_alphabetical_match:
                ...
                duration_days = self.convert_to_days(term_dict.get(term_word), term_alphabetical_match.group(2).lower())
        
        return initial_investment, duration_days
    
    def find_best_company(self, initial_investment, duration_days):
        ...
        "LIQUIDADD": "DSP Mutual Fund - DSP S&P BSE Liquid Rate ETF",
        "MUTHOOTFIN": "Muthoot Finance Limited",
        "LICHSGFIN": "LIC Housing Finance Limited",
        "CHOLAFIN": "Cholamandalam Investment and Finance Company Limited",
        "HDFCBANK": "HDFC Bank Limited",
        "KRYSTAL": "Krystal Integrated Services Limited",
        "MOSMALL250": "Motilal Oswal Mutual Fund - Motilal Oswal Nifty Smallcap 250 ETF",
        "MOREALTY": "Motilal Oswal Mutual Fund - Motilal Oswal Nifty Realty ETF",
        "IRFC": "Indian Railway Finance Corporation Limited",
        "IREDA": "Indian Renewable Energy Development Agency Limited",
        "SCILAL": "Shipping Corporation of India Land and Assets Limited"
    }
        ...
        max_return = -1
        best_company = None
        investment_too_low = True  # Flag to track if investment amount is too low

        for ticker, company_name in ticker_company_mapping.items():
            ticker_with_extension = f"{ticker}.NS"
            period = '1mo'

            data = self.get_historical_data(ticker_with_extension, period)
            if data is not None:
                current_price = self.get_current_price(ticker_with_extension)
                if current_price is not None:
                    investment_value = self.calculate_expected_return(data, current_price, initial_investment, duration_days)

                    if investment_value is not None:
                        if investment_value > initial_investment:  # Check if return is higher than investment
                            investment_too_low = False  # Update flag
                            if investment_value > max_return:
        ...
        elif investment_too_low:
            print("Sorry, the investment amount is too low for any stock.")
        else:
    ...
    def main(self):
        ...
        while True:
            try:
                ...
                initial_investment, duration_days = self.extract_investment_details(user_input)

                if initial_investment is not None and duration_days is not None:
                    best_company, max_return = self.find_best_company(initial_investment, duration_days)

                    if best_company is not None:
                        print(f"The best company to invest in for a duration of {duration_days} days with an initial investment of ₹{initial_investment} is {best_company}.")
                        print(f"Expected return after {duration_days} days: ₹{max_return:.2f}")
                    else:
                        print("Sorry, unable to find the best company for your investment.")
                else:
                    print("Please provide both initial investment amount and investment duration.")
            except ValueError:
                print("Please enter a valid numeric value for the initial investment.")

if __name__ == "__main__":
    chatbot = StockInvestmentChatterbot()
